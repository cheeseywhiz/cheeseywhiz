...
def escape(complexNum,limit,accuracy):#number of times it takes the iterated function to exceed the limit at the given complex point
	z=0
	n=0
...
def imgEscape(xmin,xmax,ymin,ymax,limit,accuracy,w,h):
	...
	for x in range(w):
		for y in range(h):
			n=escape((x*(xmax-xmin)/w+xmin)+(y*(ymin-ymax)/h+ymax)*1j,limit,accuracy)
			if n==0:
				pixel[x,y]=(0,0,0)
			else:
				color=(256/math.pi)*math.acos(math.cos((math.pi/38)*n))
