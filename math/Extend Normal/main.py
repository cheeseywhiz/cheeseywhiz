from math import atan,cos,sin,exp,fabs
...
def offset(f,r,show=True):
    h = 1e-6
    domain = [n*dx_ for n in range(int(xmin/dx_),int(xmax/dx_)+1) if in_domain(f,n*dx_)]
    original = [(x,f(x)) for x in domain if ymin <= f(x) <= ymax]
    offset_negative = []
    offset_positive = []
    for x,y in original:
        theta = atan(-h/(f(x+h)-f(x)))  # perpendicular to definition of derivative represented in radian measure
